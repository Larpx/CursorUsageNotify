...
namespace Larpx.PersonalTools.CursorUsageNotify.Services.Http
{
    ...
    {
        /// <summary>
        /// 测试连接是否可用（拉取 1 条事件验证 token 有效性）。
        /// </summary>
        Task<Result<int>> TestConnectionAsync(string sessionToken, CancellationToken ct = default);

        /// <summary>
        /// 分页拉取详细用量事件（POST /api/dashboard/get-filtered-usage-events）。
        /// </summary>
